...
void lexical_error(int x){
	...
	printf("Lexical Error caracter no válido %d\n",x);
}
...
void match(token t){
	...
	if (flag_next_token==0){
	    ...
	    if (auxiliar==t){
	    	current_token=t;
	    	flag_next_token=0; //Se puede pasar a leer el siguiente token
	    	flag=1; //Si hizo match
	    }
	    else{
	    	flag=0; //No hizo match
	    }
	}
	else{
		if (next==t){
...
token next_token(){
	...
	if (flag==1){
		flag_next_token=1; //No se puede pasar a leer el siguiente token
...
void generate(string dec,string s,string tipo,string espacio){
	...
	fprintf(archivoSalida,"%s ",dec);
	fprintf(archivoSalida,"%s ",s);
	fprintf(archivoSalida,"%s ",tipo);
	fprintf(archivoSalida,"%s ",espacio);
}
...
//recibe el archivo a leer por medio de un paramtro
int main(int argc, char const *argv[])
{	
	

   	archivoSalida = fopen ("SalidaIntermedia.txt", "w+");	
